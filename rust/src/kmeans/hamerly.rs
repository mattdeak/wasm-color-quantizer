...
pub fn kmeans_hamerly(data: &[ColorVec], config: &KMeansConfig) -> (Clusters, Centroids) {
    let (mut centroids, mut centroid_sums, mut centroid_counts, mut upper_bounds, mut lower_bounds, mut clusters) = initialize_hamerly(data, config);
    ...
    for _ in 0..config.max_iterations {
        ...
        for i in 0..num_pixels {
            let m = upper_bounds[i].max(upper_bounds[clusters[i]] / 2.);
            if upper_bounds[i] > m {
            // tighten upper bound here
                upper_bounds[i] = euclidean_distance_squared(&centroids[clusters[i]], &data[i]);
                if upper_bounds[i] > m {
                    let (best_distance, second_best_distance, best_index) = find_best_and_second_best(&centroids, &data[i]);
        ...
        for (j, (current_centroid, new_centroid)) in centroids.iter().zip(new_centroids.iter_mut()).enumerate() {
...
fn initialize_hamerly(data: &[ColorVec], config: &KMeansConfig) -> (Centroids, CentroidSums, CentroidCounts, UpperBounds, LowerBounds, Clusters) {
    // indicex of the cluster each pixel belongs to
    let centroids = initialize_centroids(data, config.k);
    ...
    (centroids, centroid_sums, centroid_counts, upper_bounds, lower_bounds, clusters)
}
...
fn update_bounds(upper_bounds: &mut UpperBounds, lower_bounds: &mut LowerBounds, distances: &[f32], clusters: &[usize]) {
